...
int main() {
	...
	sessionList = init_session(sessionList, 5);

	User *user1 = calloc(sizeof(User), 1);
	strcpy(user1 -> uname, "JonnyKong1");
	User *user2 = calloc(sizeof(User), 1);
	strcpy(user1 -> uname, "JonnyKong2");
	User *user3 = calloc(sizeof(User), 1);
	strcpy(user1 -> uname, "JonnyKong3");

	if(isValidSession(sessionList, 1)) printf("1 in session\n"); 
	if(isValidSession(sessionList, 4)) {
		printf("4 in session\n"); 
	} else {
		printf("4 not in session\n");
	}		
	if(isValidSession(sessionList, 7)) printf("7 in session\n"); 
